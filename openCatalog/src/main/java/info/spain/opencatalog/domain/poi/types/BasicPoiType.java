...
public class BasicPoiType {
     ...
     * Si se desean categorías de datos, hay que especificarlo en el key separando por ":"
     * Ej.: Para estación de esquí, categoría alpino, dato número de pistas verdes
     * 		"alpino:numero-pistas-verdes"
     * */
	...
	private void validateData(Map<String,String> data){
		...
		for (String key : data.keySet()) {
			Preconditions.checkArgument(validKeys.contains(key),  key + " is not a valid data for this type. Allowed data :" + validKeys);
	...
	public BasicPoiType setAllowedDataValidator(String key, DataValidator dataValidator){
		Preconditions.checkArgument(!Strings.isNullOrEmpty(key), "key can't be empty");
		Preconditions.checkArgument(dataValidator != null, "Validator can't be empty");
		if (allowedDataValidators == null){
