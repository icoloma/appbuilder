...
public interface PoiRepository extends MongoRepository<AbstractPoi, String>, PoiRepositoryCustom {
	...
	public Page<AbstractPoi> findByNameEsLikeIgnoreCase(@Param("name") String name, Pageable pageable);
	
	@RestResource(path = "byName", rel="byName")    
	public Page<AbstractPoi> findByNameEs(@Param("name") String name, Pageable pageable);
	
	// { 'location' : { '$nearSphere' : [location.lng, location.lat], '$maxDistance' : distance}}
	@RestResource(path = "byLocationNear", rel="byLocationNear")
	public Page<AbstractPoi> findByLocationNear(@Param("location") @ConvertWith(value=PointConverter.class) Point location, @Param("distance") @ConvertWith(value=DistanceConverter.class) Distance distance, Pageable pageable);
	
	@RestResource( path="byArea1Like", rel="byArea1Like")
	@Query( value="{'address.adminArea1':{'$regex':?0, '$options': 'i'}}")
	public List<AbstractPoi> findByAddressArea1LikeIgnoreCase(@Param("adminArea1") String name);
	
	@RestResource( path="byArea1", rel="byArea1")
	@Query( value="{'address.adminArea1':?0}")
	public List<AbstractPoi> findByAddressArea1(@Param("adminArea1") String name);

	@RestResource( path="byArea2Like", rel="byArea2Like")
	@Query( value="{'address.adminArea2':{'$regex':?0, '$options': 'i'}}")
	public List<AbstractPoi> findByAddressArea2LikeIgnoreCase(@Param("adminArea2") String name);
	
	@RestResource( path="byArea2", rel="byArea2")
	@Query( value="{'address.adminArea2':?0}")
	public List<AbstractPoi> findByAddressArea2(@Param("adminArea2") String name);
	
	@RestResource( path="byId", rel="byId")
	@Query( value="{ '_id' : {'$in': ?0}}")
	public List<AbstractPoi> findByIds(@Param("id") String[] ids);
